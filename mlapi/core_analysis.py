...
def core_analysis(file_name, use_google_speech):
    ...
    for turn, _, speaker in diarization.itertracks(yield_label=True):
        ...
        if end - start > 30 and use_google_speech:
            sub_end = start + 30
            while sub_end <= end:
    ...
    for spk_tag, spk_segments in speakers.items():
        ...
        if spk in doctors:
            doctor_segment = wav_file_segmentation_doc(file_name, spk_segments, use_google_speech)
            speakers[spk_tag] = doctor_segment
        else:
            details = wav_file_segmentation_patient(file_name, spk_segments, use_google_speech)
